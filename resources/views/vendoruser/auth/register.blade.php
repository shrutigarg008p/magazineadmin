        ...
        <div class="card">
            <div class="card-body login-card-body">
                <div class="text-center">
                    @if (session('success'))
                        <p class="badge badge-success font-italic text-md text-wrap">{{ session('success') }}</p>
                    @else
                        <p class="badge badge-danger font-italic text-md text-wrap">{{ session('error') }}</p>
                    @endif
                </div>
                <p class="login-box-msg">Register a new membership</p>
                <form action="{{ route('vendor.registered') }}" method="post" autocomplete="off">
                    @csrf
                    ...
                    <div class="row">
                        <div class="col-8">
                            <div class="icheck-primary">
                                ...
                                <label for="terms">
                                    I agree to the <a href="{{route('vendor.terms')}}">Terms and Conditions</a>
                                </label>
                                @error('terms')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                ...
                <p class="mb-1 mt-3">
                    Already have account? <a href="{{ route('vendor.index') }}">Sign In</a>
                </p>
            </div>
            <!-- /.login-card-body -->
        </div>
