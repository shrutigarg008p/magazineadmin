...
class MagazineResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        ...
        if( $currency != 'GHS' && $user && $user->isIos ) {
            $price = $this->publication->newspaper_price_usd; // pardon naming convention
        } else {
            $price = $this->price;
        }

        // $bookmark_status = UserBookmark::where('user_id',$this->user->id)->where('pid',$this->id)->where('type','magazine')->exists();
        return array_merge([
            'id' => $this->id,
            ...
            'bookmark' =>  $user ? UserBookmark::where('user_id',$user->id)->where('pid',$this->id)->where('type','magazine')->exists() : false,
